...
def peakpickingVOP(evidence,VLROP_th):
    ...
    npz_indices1 = np.where(npz_spu!=0)[0].astype(int)
    ...
    if max(evidence[0: ind_npz_final[0]]) == 0:
        ...
        temp_2 = evidence[0:ind_npz_final[0]]
        ind_temp_2 = min(np.where(temp_2 == max(temp_2))[0])
